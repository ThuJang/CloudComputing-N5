 ...
 */
define( 'MO_FIREBASE_AUTHENTICATION_VERSION', '1.6.8' );
...
class Miniorange_Firebase_Authentication {
	...
	public function mo_firebase_auth( $default_user, $username, $password ) {
		...
		if ( isset( $fb_user['idToken'] ) ) {
			...
			if ( 'INVALID_EMAIL' === $error_message || 'EMAIL_NOT_FOUND' === $error_message || 'INVALID_LOGIN_CREDENTIALS' === $error_message ) {
		...
		if ( isset( $_POST['option'] ) ) {
			...
			if ( sanitize_text_field( wp_unslash( $_POST['option'] ) ) === 'mo_firebase_authentication_register_customer' && isset( $_REQUEST['mo_fb_register_form_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_fb_register_form_nonce'] ) ), 'mo_fb_register_form' ) ) { // register the admin to miniOrange
				...
				if ( $deactivate_reason ) {
					...
					$contact_us = new MO_Firebase_contact_us();
					$response   = $contact_us->mo_firebase_auth_send_email_alert( $email, $message, 'Feedback: WordPress Firebase Authentication' );
					if ( ! is_null( $response ) ) {
				...
				if ( current_user_can( 'administrator' ) ) {
					...
					if ( $this->mo_firebase_authentication_check_empty_or_null( $email ) || $this->mo_firebase_authentication_check_empty_or_null( $query ) ) {
						update_option( 'message', 'Please fill up Usecase, Email field to submit your query.' );
						$$this->mo_firebase_auth_show_error_message();
					} else {
						global $wp_version;
						$mo_firebase_sandbox_usecase_with_addons = 'Usecase: ' . PHP_EOL .
										$query .
										PHP_EOL .
										' ' .
										'Firestore Integrator: ' . PHP_EOL . $firestore_check;
						$wp_version_trim                         = substr( $wp_version, 0, 3 );
						$mo_oauth_sandbox_href                   = 'https://sandbox.miniorange.com/?email=' . rawurlencode( $email ) . '&mo_plugin=mo_firebase_authentication&wordpress_version=' . rawurlencode( $wp_version_trim ) . '&usecase=' . rawurlencode( $mo_firebase_sandbox_usecase_with_addons ) . '&referer=' . rawurlencode( get_site_url() );
						echo '<script type="text/javascript">
							window.open("' . esc_url_raw( $mo_oauth_sandbox_href ) . '", "_blank");
						</script>';
					}
				}
			}
